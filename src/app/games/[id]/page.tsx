...
export default async function GamePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ preview?: string }> }) {
  ...
  return (
    <main className="app-shell narrow">
      ...
      <section className="challenge-hero"><p className="eyebrow">Game {String(game.id).padStart(2, "0")} · {preview ? `Preview · ${state.status}` : state.status}</p><h1>{game.title}</h1><p>{game.summary}</p>{state.status === "completed" && <p className="closed-notice">This game is complete. Directions remain available, but game actions are closed.</p>}</section>
      <section className="panel rules"><h2>How to play</h2><ol>{game.rules.map((rule) => <li key={rule}>{rule}</li>)}</ol><h2>Points</h2><ul>{game.points.map((point) => <li key={point}>{point}</li>)}</ul>
        {game.id === 3 && state.status === "live" && state.external_url && <a className="button-link" href={state.external_url} target="_blank" rel="noreferrer">Open Kahoot ↗</a>}
      </section>
    </main>
  );
}
